...
def fetch_embed_player_document(embed_url: str, page_url: str) -> tuple[str | None, bool]:
    """Return player HTML and whether the embed is confirmed unavailable.

    Nafak pages usually expose iframe/embed servers on the source page. The
    direct m3u8/mp4 and duration often live one level deeper inside the embed
    player, so this mirrors the Shrmha online scraper: fetch the embed page,
    then try the common XFileSharing /dl POST. If the POST fails, keep the
    embed GET document because it may already contain packed player metadata.
    """
...
def extract_player_metadata(player_document: str, base_url: str) -> tuple[str | None, int | None, int | None]:
...
def quality_label(label: str | None, height: int | None) -> str:
    base = Nafak.clean_text(label) or "Direct video"
