...
except Exception:  # pragma: no cover - allows local fixture smoke tests
...
IGNORED_IFRAME_HOST_PARTS = {
    ...
    "google.com",
    "doubleclick.net",
    "straitpaltrybelief.com",
    "diagnosedecorationvideotape.com",
    "agroundrecentwalk.com",
}
...
def meta_content(document: str, key: str) -> str | None:
    patterns = [
        rf'<meta\b[^>]*(?:property|name|itemprop)=(["\']){re.escape(key)}\1[^>]*content=(["\'])(?P<value>.*?)\2',
        rf'<meta\b[^>]*content=(["\'])(?P<value>.*?)\1[^>]*(?:property|name|itemprop)=(["\']){re.escape(key)}\3',
    ]
...
def strip_site_suffix(title: str | None) -> str | None:
    if not title:
        return None
    for suffix in ("سكس مصري", "EgyZeb"):
        title = re.sub(rf"\s*[-–—|]\s*{re.escape(suffix)}\s*$", "", title, flags=re.IGNORECASE).strip()
...
def _fetch_egyzeb_text(url: str, *, referer: str = SOURCE_URL) -> str:
...
def _fetch_text_with_retries(url: str, *, referer: str = SOURCE_URL, attempts: int = 4, delay: float = 2.0) -> str:
    ...
    for attempt in range(1, attempts + 1):
        if attempt == 1:
            ...
            log(f"FETCH retry {attempt}/{attempts} {url}: {last_exc}")
        try:
            return _fetch_egyzeb_text(url, referer=referer)
    ...
    raise RuntimeError(f"failed to fetch EgyZeb page after retries: {url}: {last_exc}")
...
def remove_generic_categories(values: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        value
        for value in values
...
def extract_taxonomy_from_article_class(document: str, base_url: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    article_match = re.search(r'<article\b(?P<attrs>[^>]*)>', document, re.IGNORECASE | re.DOTALL)
    if not article_match:
        return [], []
    class_name = attr_value(article_match.group("attrs"), "class") or ""
    tags: list[dict[str, Any]] = []
    categories: list[dict[str, Any]] = []
    for token in class_name.split():
        if token.startswith("tag-"):
            tags.append({"name": token.removeprefix("tag-")})
        elif token.startswith("category-"):
            categories.append({"name": token.removeprefix("category-")})
    return unique_dicts_by_name(tags), remove_generic_categories(unique_dicts_by_name(categories))


def extract_class_block(document: str, class_name: str) -> str | None:
    match = re.search(
        rf'<(?:div|span|section)\b[^>]*class=["\'][^"\']*\b{re.escape(class_name)}\b[^"\']*["\'][^>]*>(?P<body>.*?)</(?:div|span|section)>',
        document,
        re.IGNORECASE | re.DOTALL,
    )
...
def extract_article_blocks(document: str) -> list[str]:
    blocks: list[str] = []
    for match in re.finditer(
        r'<article\b[^>]*class=["\'][^"\']*\bthumb-block\b[^"\']*["\'][^>]*>.*?</article>',
        document,
        re.IGNORECASE | re.DOTALL,
    ):
    ...
    if blocks:
        return blocks
    starts = [match.start() for match in re.finditer(r'<article\b[^>]*class=["\'][^"\']*\bpost\b[^"\']*', document, re.IGNORECASE)]
    for index, start in enumerate(starts):
        end = starts[index + 1] if index + 1 < len(starts) else len(document)
        block = document[start:end]
        if "thumb-block" in block:
            blocks.append(block)
    return blocks
...
def extract_listing_candidates(document: str, page_url: str) -> list[dict[str, Any]]:
    ...
    seen: set[str] = set()
    for position, block in enumerate(extract_article_blocks(document)):
        link_match = re.search(r"<a\b(?P<attrs>[^>]*)>(?P<body>.*?)</a>", block, re.IGNORECASE | re.DOTALL)
        if not link_match:
            continue
        ...
        title = first_non_empty(attr_value(attrs, "title"), re.search(r'<span\b[^>]*class=["\'][^"\']*\btitle\b[^"\']*["\'][^>]*>(?P<body>.*?)</span>', block, re.IGNORECASE | re.DOTALL).group("body") if re.search(r'<span\b[^>]*class=["\'][^"\']*\btitle\b[^"\']*["\'][^>]*>(?P<body>.*?)</span>', block, re.IGNORECASE | re.DOTALL) else None)
        image = None
        thumb_match = re.search(r'<div\b(?P<attrs>[^>]*)>', block, re.IGNORECASE | re.DOTALL)
        if thumb_match:
            data_thumbs = attr_value(thumb_match.group("attrs"), "data-thumbs")
            if data_thumbs:
                image = absolute_url(page_url, data_thumbs.split(",")[0])
        image_match = re.search(r"<img\b(?P<attrs>[^>]*)>", block, re.IGNORECASE | re.DOTALL)
        if image_match:
            image_attrs = image_match.group("attrs")
            image = image or absolute_url(page_url, attr_value(image_attrs, "data-src") or attr_value(image_attrs, "src"))
            title = title or clean_text(attr_value(image_attrs, "alt"))
        if image and "/assets/img/px.gif" in image:
            image = None

        tag_values, performer_values = extract_taxonomy_from_article_class(block, page_url)
...
def extract_post_block(document: str) -> str | None:
    match = re.search(r'<article\b[^>]*itemtype=["\'][^"\']*VideoObject[^"\']*["\'][^>]*>(?P<body>.*?)(?:<div\b[^>]*id=["\']comments["\']|</article>)', document, re.IGNORECASE | re.DOTALL)
    if match:
        return match.group("body")
    match = re.search(r'<div\b[^>]*class=["\'][^"\']*\bvideo-player-area\b[^"\']*["\'][^>]*>(?P<body>.*?)(?:<div\b[^>]*class=["\'][^"\']*\bvideo-actions\b|</article>)', document, re.IGNORECASE | re.DOTALL)
    return match.group("body") if match else None


def extract_direct_video_urls(document: str, base_url: str) -> list[str]:
    post_block = extract_post_block(document) or document
    urls: list[str] = []
    content = meta_content(post_block, "contentURL") or meta_content(document, "contentURL")
    if content:
        urls.append(urljoin(base_url, html.unescape(content)))
    for match in re.finditer(r"<source\b(?P<attrs>[^>]*)>", post_block, re.IGNORECASE | re.DOTALL):
        src = attr_value(match.group("attrs"), "src")
        if src:
            urls.append(urljoin(base_url, src))
    for match in re.finditer(r"<video\b(?P<attrs>[^>]*)>", post_block, re.IGNORECASE | re.DOTALL):
        src = attr_value(match.group("attrs"), "src")
        if src:
            urls.append(urljoin(base_url, src))
...
def scene_taxonomy_from_jsonld(article_obj: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    tags: list[dict[str, Any]] = []
    ...
    if isinstance(keywords, list):
        ...
        tags = unique_dicts_by_name([{"name": item.strip()} for item in re.split(r"[,،]", keywords) if item.strip()])
    ...
    return tags, remove_generic_categories(performers)
...
def parse_scene_page(document: str, page_url: str) -> dict[str, Any]:
    article_obj = first_jsonld_object(document, "Article", "VideoObject")
    ...
    title = strip_site_suffix(
        first_non_empty(
            ...
            article_obj.get("headline") or article_obj.get("name"),
            web_page_obj.get("name"),
            title_match(document, "h1"),
    ...
    date = parse_date(
        ...
        or meta_content(document, "uploadDate")
    )
    external_id = extract_post_id(document, canonical_url)

    direct_urls = extract_direct_video_urls(document, canonical_url)
    embed_urls = extract_iframe_links(document, canonical_url)
    streams: list[dict[str, Any]] = []
    for position, video_url in enumerate(direct_urls):
        label = "Direct video" if position == 0 else f"Direct video {position + 1}"
        streams.append(make_stream(label, "direct", video_url, len(streams), position == 0))
    for embed_url in embed_urls:
        streams.append(make_stream(f"Embed {len(streams) + 1}", "embed", embed_url, len(streams), not streams))

    tags, performers = scene_taxonomy_from_jsonld(article_obj)
    class_tags, class_performers = extract_taxonomy_from_article_class(document, canonical_url)
    if not tags:
        tags = class_tags
    if not performers:
        performers = class_performers
    ...
    online_media = drop_empty(
        {
            ...
            "direct_video_url": direct_urls[0] if direct_urls else None,
            "embed_url": embed_urls[0] if embed_urls else None,
            "thumbnail_url": image,
            "streams": streams,
        }
    )
    online_media["raw_metadata_json"] = raw_metadata(
        {
            ...
            "media_layer": "direct_wp_video_or_embed_discovery",
            "direct_video_count": len(direct_urls),
            "embed_stream_count": len(embed_urls),
            "jsonld_article": bool(article_obj),
            "performers_from_jsonld_or_article_class": bool(performers),
        }
    )
...
def parse_source_page(document: str, page_url: str) -> dict[str, Any]:
    canonical_url = absolute_url(page_url, link_href(document, "canonical")) or meta_content(document, "og:url") or page_url
    title = strip_site_suffix(first_non_empty(meta_content(document, "og:title"), title_match(document, "h1"), title_match(document, "title")))
...
def scrape_source_by_url(url: str, *, limit: int | None = None, max_pages: int = 1) -> dict[str, Any]:
    ...
    while queue and len(visited_pages) < max_pages:
        ...
        page_no = len(visited_pages) + 1
        log(f"SOURCE page {page_no}/{max_pages} fetch {page_url} current_candidates={len(candidates)} queue={len(queue)}")
        try:
        ...
        page_candidates = page_output.get("scene_candidates") or []
        page_next = page_output.get("pagination_urls") or []
        log(f"SOURCE page {page_no}/{max_pages} parsed page_candidates={len(page_candidates)} pagination_links={len(page_next)} total_before_add={len(candidates)}")
        if output is None:
            output = page_output
