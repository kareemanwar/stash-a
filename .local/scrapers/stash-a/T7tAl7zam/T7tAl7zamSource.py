...
def strip_tags(value: str | None) -> str:
    if not value:
        return ""
    return T7tAl7zam.clean_text(re.sub(r"<[^>]+>", " ", html.unescape(value)))
...
def normalize_preview_date(value: str | None) -> str | None:
    text = T7tAl7zam.clean_text(value)
    if not text:
        return None
...
def first_query_value(query: dict[str, list[str]], key: str) -> str | None:
    value = T7tAl7zam.first(query.get(key))
...
def canonical_source_url(url: str) -> str:
    ...
    return urlunparse(
        (
            ...
            parsed.netloc or urlparse(T7tAl7zam.STUDIO_URL).netloc,
            path or "/",
            "",
...
def is_site_root(url: str) -> bool:
    ...
    root = urlparse(T7tAl7zam.STUDIO_URL)
...
def extract_query(url: str, document: str) -> str | None:
    ...
    if query_value:
        return T7tAl7zam.clean_text(unquote_plus(query_value[0]))
...
def extract_source_title(url: str, document: str) -> str:
    ...
    if query:
        return f"{query} - {T7tAl7zam.STUDIO_NAME}"
    ...
    title = re.sub(r"\s+-\s+تحت الحزام\s*$", "", title).strip()
    return title or T7tAl7zam.STUDIO_NAME
...
def extract_external_id(candidate_url: str, block: str) -> str | None:
    ...
    query_id = T7tAl7zam.first(parse_qs(parsed.query).get("p"))
    if query_id:
        return str(query_id)
...
def parse_preview_block(block: str, base_url: str, position: int) -> dict[str, Any] | None:
    ...
    candidate: dict[str, Any] = {
        ...
        "details": title,
        "studio": {
            "name": T7tAl7zam.STUDIO_NAME,
            "urls": [T7tAl7zam.STUDIO_URL],
            "remote_site_id": T7tAl7zam.STUDIO_SLUG,
        },
...
def crawl_listing_pages(
    ...
    while queued:
        ...
        document = T7tAl7zam.fetch_html(page_url)
        seen.add(page_url)
    ...
    for tag_list in tag_lists:
        ...
        for tag in tag_list:
            ...
            if not isinstance(name, str):
                continue
            name = T7tAl7zam.clean_text(name)
            if not name or name in seen:
                continue
            seen.add(name)
            tags.append({"name": name})
...
def run_scene_by_url(url: str) -> dict[str, Any]:
    ...
    script_path = script_dir / "T7tAl7zamOnline.py"
    env = os.environ.copy()
...
def scrape_source_by_url(
    ...
    if not page_documents:
        return {"error": f"No T7tAl7zam source pages could be fetched for {url}"}
    ...
    if query:
        ...
        remote_site_id = urlparse(source_url).path or T7tAl7zam.STUDIO_SLUG
    ...
    result: dict[str, Any] = {
        "title": source_title,
        "urls": [source_url],
        "details": f"{T7tAl7zam.STUDIO_NAME} source scraped from {source_url}",
